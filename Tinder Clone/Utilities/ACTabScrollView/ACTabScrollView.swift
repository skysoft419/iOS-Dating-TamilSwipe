...
open class ACTabScrollView: UIView, UIScrollViewDelegate {
    ...
    fileprivate func initialize() {
        ...
        tabSectionScrollView.decelerationRate = UIScrollViewDecelerationRateFast
        let decelerationRate = 0.5
        tabSectionScrollView.setValue(NSValue(cgSize: CGSize(width: decelerationRate, height: decelerationRate)), forKey: "_decelerationFactor")
...
//    }
    ...
    public func currentPageIndex() -> Int {
        ...
        if let prevPage = prevPageIndex {
            if(currentPageIndex > prevPage){
                currentPageIndex = prevPage + 1
            }else if(currentPageIndex < prevPage){
                currentPageIndex = prevPage - 1
            }
        }
