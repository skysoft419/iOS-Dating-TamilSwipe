...
public class SLPagingViewSwift: UIViewController, UIScrollViewDelegate {
    ...
    public var currentindexSelected: Int          = 0
    ...
    public init(items: [UIView], views: [UIView], showPageControl: Bool, navBarBackground: UIColor) {
        ...
        for (i, v) in items.enumerated() {
            ...
            {
            v.frame           = CGRect(x:originX, y:18, width:vSize.width, height:vSize.height)
            }
            else
            {
                v.frame           = CGRect(x:originX, y:18, width:vSize.width, height:vSize.height)
            ...
            v.tag             = i
            let tap  = UITapGestureRecognizer(target: self, action:#selector(self.tapOnHeader(recognizer:)))
    ...
    public override func viewWillLayoutSubviews() {
        ...
        self.navigationBarView.frame = CGRect(x:0, y:0, width:self.SCREENSIZE.width, height:50)
    }
    ...
    @objc func tapOnHeader(recognizer: UITapGestureRecognizer){
    ...
    public func scrollViewDidScroll(_ scrollView: UIScrollView) {
        ...
        let _xOffset      = Float(scrollView.contentOffset.x)
        let currentIndex = (Int(roundf(_xOffset)) % (self.navigationBarView.subviews.count * Int(self.SCREENSIZE.width))) / Int(self.SCREENSIZE.width)
...
extension UILabel {
    func _slpGetSize() -> CGSize? {
        return (text as NSString?)?.size(withAttributes: [kCTFontAttributeName as NSAttributedStringKey: font])
    }
}
