...
public class UIStrokeAnimatedLabel: UILabel {
    ...
    private func layer(for text: String) -> CALayer? {
        ...
        if layers.count > 0 {
             groupLayer.bounds = CGRect(x: layers[0].frame.origin.x, y: layers[0].frame.origin.y, width: layers[layers.count-1].frame.maxX, height: layers[0].frame.size.height)
        }
